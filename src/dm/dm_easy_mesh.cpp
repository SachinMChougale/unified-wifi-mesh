...
#define KEY_CHANNEL_SELECTION "wfa-dataelements:ChannelSelectionRequest"
    ...
    if (strncmp(key, KEY_CHANNEL_ANTICIPATED, strlen(KEY_CHANNEL_ANTICIPATED)) == 0) {
        ...
        em_printfout("Parsing ChannelSelectionRequest");
    ...
    *num = static_cast<unsigned int> (cJSON_GetArraySize(dev_arr_obj));
    ...
    if (type == em_op_class_type_scan_param || type == em_op_class_type_selection) {
        ...
        // Fix RDKB CLi issue, where anticipated preference is sent as network level entries
        if ((target_arr_obj = cJSON_GetObjectItem(net_obj, target_key)) == NULL) {
            em_printfout("'%s' not found in Network, trying Device level", target_key);
            cJSON_Delete(parent_obj);
            return EM_PARSE_ERR_GEN;
        }
    }
    ...
    for (i = 0; i < arr_size; i++) {
        ...
        // Support of type type selection will be used in upcoming updates
        m_op_class[m_num_opclass].m_op_class_info.id.type = (type == em_op_class_type_selection) ? em_op_class_type_anticipated : type;
        ...
        if (type != em_op_class_type_scan_param) {
            ...
            // For TR181 request belongs to only one radio, so copying same for for all opclass entries as ID
            if (type == em_op_class_type_selection) {
                memcpy(m_op_class[m_num_opclass].m_op_class_info.id.ruid, m_radio[0].m_radio_info.intf.mac, sizeof(mac_addr_t));
            }
