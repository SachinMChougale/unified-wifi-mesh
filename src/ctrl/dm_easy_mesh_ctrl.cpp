...
bus_error_t em_ctrl_t::cmd_channelselect(const char *method_name, const bus_data_prop_t *input_params, bus_data_prop_t **output_params, void *async_handle)
{
    ...
    int previous_class_instance = 0;
    int previous_channel_instance = 0;
    ...
    for (prop = input_params; prop; prop = prop->next_data) {
        ...
        if (token_count >= 2 && strcmp(tokens[0], "Class") == 0) {
            ...
            if (end == tokens[1] || *end != '\0' || errno == ERANGE || class_index_l < 1 || class_index_l > INT_MAX) {
                em_printfout("Invalid class index in '%s'", prop->name);
                rc = bus_error_invalid_input;
                goto finish;
            }
            int class_index = static_cast<int>(class_index_l);
            if (class_index != previous_class_instance + 1) {
                em_printfout("Class instances must be sequential from 1 (expected Class.%d got Class.%d)", previous_class_instance + 1, class_index);
                rc = bus_error_invalid_input;
                goto finish;
            }
            if (class_index != previous_class_instance) {
                previous_class_instance = class_index;
                previous_channel_instance = 0;
            }
            ...
            if (token_count == 3 && strcmp(tokens[2], "OpClass") == 0) {
                ...
                char *end = NULL;
                errno = 0;
                long channel_instance_l = strtol(tokens[3], &end, 10);
                if (end == tokens[3] || *end != '\0' || errno == ERANGE || channel_instance_l < 1 || channel_instance_l > INT_MAX) {
                    em_printfout("Invalid channel index in '%s'", prop->name);
                    rc = bus_error_invalid_input;
                    goto finish;
                }
                int channel_instance = static_cast<int>(channel_instance_l);
                if (channel_instance != previous_channel_instance + 1) {
                    em_printfout("Channel instances for Class.%d must be sequential from 1 (expected Channel.%d got Channel.%d)", class_index, previous_channel_instance + 1, channel_instance);
                    rc = bus_error_invalid_input;
                    goto finish;
                }
                if (channel_instance != previous_channel_instance) {
                    previous_channel_instance = channel_instance;
                }
    ...
    // Validate that all Class.N instances are present contiguously from 1.
    for (int class_index = 1; class_index <= highest_class_instance; class_index++) {
        if (classes[class_index - 1].op_class == -1 && classes[class_index - 1].num_channels == 0 && classes[class_index - 1].highest_channel_instance == 0) {
            em_printfout("Missing Class.%d instance (indexes must be contiguous from 1)", class_index);
            rc = bus_error_invalid_input;
            goto finish;
        }
    }
    ...
    for (int class_index = 1; class_index <= highest_class_instance; class_index++) {
        ...
        // Validate that all Channel.M instances are contiguous starting from 1
        for (int channel_index = 1; channel_index <= cls.highest_channel_instance; channel_index++) {
            ...
            if (cls.channels[channel_slot].channel == -1 && cls.channels[channel_slot].preference == -1) {
                em_printfout("Missing Channel.%d instance for Class.%d (indexes must be contiguous from 1)",
                             channel_index, class_index);
                rc = bus_error_invalid_input;
                goto finish;
            }
        }
